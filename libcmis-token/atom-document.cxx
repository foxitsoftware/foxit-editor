...
boost::shared_ptr< istream > AtomDocument::getContentStream( string /*streamId*/ ) throw ( libcmis::Exception )
{
    ...
    try
    {
		///begin 支持我们cmis服务器，跳转S3下载内容
		string contentUrl = m_contentUrl;
		if ( contentUrl.find( '?' ) != string::npos )
			contentUrl += "&";
		else
			contentUrl += "?";
		contentUrl += "redirectFollow=1";
		///end
		getSession()->setNeedProgressValue(true);		
        stream = getSession()->httpGetRequest( contentUrl )->getStream( );
		getSession()->setNeedProgressValue(false);
    }
    catch ( const CurlException& e )
    {
        throw e.getCmisException( );
    ...
    do
    {
        ...
        catch ( const CurlException& e )
        {
            ...
            if ( !tryBase64 && e.getHttpStatus() == 400 )
                tryBase64 = true;
			else if (e.getErrorCode() == CURLE_COULDNT_CONNECT)
			{
				throw libcmis::Exception(e.getErrorMessage(), "CURLE_COULDNT_CONNECT");
			}
            else
                throw e.getCmisException( );
        }
...
void AtomDocument::setDocProgressCallback(progressCallback pProgressCallback)
{
	getSession()->setProgressCallBack(pProgressCallback);
}
