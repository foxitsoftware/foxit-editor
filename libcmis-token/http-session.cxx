...
extern std::string g_CAINFO_FILEPATH;
...
progressCallback HttpSession::m_pProgressCallBack = NULL;
HttpSession::HttpSession( string username, string password, bool noSslCheck,
	...
	m_bNeedProgress(false)
{
    //curl_global_init( CURL_GLOBAL_ALL );
    m_curlHandle = curl_easy_init( );

    if ( (oauth2 && oauth2->isComplete( )) 
		|| (oauth2 && oauth2->isUseToken()) )
	{
...
libcmis::HttpResponsePtr HttpSession::httpGetRequest( string url ) throw ( CurlException )
{
		...
		&& (m_pProgressCallBack))
	{
		...
		curl_easy_setopt(m_curlHandle, CURLOPT_PROGRESSFUNCTION, m_pProgressCallBack);
	}
...
void HttpSession::httpRunRequest( string url, vector< string > headers, bool redirect ) throw ( CurlException )
{
    ...
    if ( m_noSSLCheck || g_CAINFO_FILEPATH.empty() )
    {
        ...
        curl_easy_setopt(m_curlHandle, CURLOPT_CAINFO, g_CAINFO_FILEPATH.c_str());
	}
...
void HttpSession::setOAuth2Data( libcmis::OAuth2DataPtr oauth2 ) throw ( libcmis::Exception )
{
	...
	m_oauth2Handler = make_shared<OAuth2Handler>(this, oauth2);
}
